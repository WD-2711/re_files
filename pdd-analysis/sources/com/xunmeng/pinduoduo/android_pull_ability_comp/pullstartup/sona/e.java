...
public class e {
    ...
    public void a(Activity activity, SonaRequest sonaRequest, int i) {
        ...
        ActivityUtils.instance().startActivityForResult("dd", activity, this.c.c(sonaRequest.getIntent()), i);
    }

    public status a(SonaRequest sonaRequest, SonaConfig sonaConfig) {
        Intent intent = sonaRequest.getIntent();
        status a2 = this.c.a(sonaRequest.getCaller());
        if (!a2.getBoolean()) {
            Logger.i("SpecialPullAbility.Comp.SonaStartUp", "not satisfy condition: " + a2);
            return a2;
        } else if (this.d.a(intent)) {
            return !this.c.b(intent) ? new status(false, "preCheck_fail") : b(sonaRequest, sonaConfig);
        } else {
